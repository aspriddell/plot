...
module public Lexer =
    ...
    let rec private scan input =
        ...
        | c :: tail when System.Char.IsDigit c -> let (iStr, iVal, isFloating) = scanNumber(tail, System.Char.GetNumericValue c, false, 1.0) 
                                                  if isFloating
                                                  then NumF iVal :: scan iStr
                                                  else NumI (int64 iVal) :: scan iStr

        | _ -> failwith "Lexer error"

    let public Parse input =
        scan (input |> List.ofArray)